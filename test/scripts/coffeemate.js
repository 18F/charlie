...
describe("coffeemate", () => {
  ...
  let setup;
  const addEmojiReaction = sandbox.spy();
  const postEphemeralMessage = sandbox.spy();

  before(() => {
    setup = sinon
      .stub(utils, "setup")
      .returns({ addEmojiReaction, postEphemeralMessage });
  });
  ...
  describe("when the coffee queue is initially empty", () => {
    ...
    it("sends an ephemeral message the first time a user asks for coffee and adds them to the queue", () => {
      ...
      expect(
        addEmojiReaction.calledWith("coffee", "room id", "message id")
      ).to.equal(true);
