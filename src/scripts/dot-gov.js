...
const CISA_DOTGOV_DATA_URL = 'https://raw.githubusercontent.com/cisagov/dotgov-data/main/current-full.csv';
...
const checkDomainStatus = async (domainObj, timeout = 2000) =>
  axios.head(
    `https://${domainObj[DATA_FIELDS.NAME]}`, {
      timeout,
...
module.exports = (app) => {
  ...
  module.exports.dotGov = async ({ entity, searchTerm, say, thread }) => {
    ...
    const domains = await cache("dotgov domains", 1440, async () =>
      axios.get(CISA_DOTGOV_DATA_URL).then(response => {
